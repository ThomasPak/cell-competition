#ifndef TESTDEATHCLOCKTARGETAREAMODIFIER_HPP_
#define TESTDEATHCLOCKTARGETAREAMODIFIER_HPP_
...
class TestDeathClockTargetAreaModifier : public AbstractCellBasedTestSuite
{
private:
...
public:
    ...
    void TestNonPhaseBasedCellCycleModelMethodsAndExceptions()
    {
        ...
        // Create a DeathClockTargetAreaModifier
        MAKE_PTR(DeathClockTargetAreaModifier<2>, p_modifier);

        //p_modifier->SetReferenceTargetArea(5.0);
        p_cell->GetCellData()->SetItem("reference target area", 5.0);
    ...
    void TestDeathClockTargetAreaModifierMethods()
    {
        // Create a modifier
        ...
        for (unsigned i=0; i<cells.size(); i++)
        {
            ...
            cells[i]->GetCellData()->SetItem("reference target area", 1.0);
        }
             ...
             ++iter)
        {
            ...
            double expected_area = p_cell->GetCellData()->GetItem("reference target area");
        ...
        double expected_area_0 = p_cell_0->GetCellData()->GetItem("reference target area");
        double expected_area_1 = p_cell_1->GetCellData()->GetItem("reference target area");
        double expected_area_3 = p_cell_3->GetCellData()->GetItem("reference target area");
        ...
        expected_area_0 = ( p_cell_0->GetCellData()->GetItem("reference target area") );
    ...
    void TestTargetAreaOfDaughterCellsLinear()
    {
        ...
        p_cell->GetCellData()->SetItem("reference target area", 1.0);
    ...
    void TestDeathClockTargetAreaModifierArchiving()
    {
        // Create a file for archiving
        OutputFileHandler handler("archive", false);
        std::string archive_filename = handler.GetOutputDirectoryFullPath() + "DeathClockTargetAreaModifier.arch";

        // Separate scope to write the archive
        {
            // Initialise a growth modifier and set a non-standard mature target area
            AbstractCellBasedSimulationModifier<2,2>* const p_modifier = new DeathClockTargetAreaModifier<2>();
            (static_cast<DeathClockTargetAreaModifier<2>*>(p_modifier))->SetReferenceTargetArea(14.3);
            (static_cast<DeathClockTargetAreaModifier<2>*>(p_modifier))->SetAgeToStartGrowing(0.7);
            (static_cast<DeathClockTargetAreaModifier<2>*>(p_modifier))->SetGrowthRate(6.8);
        ...
        // Separate scope to read the archive
        {
            ...
            double reference_target_area = (static_cast<DeathClockTargetAreaModifier<2>*>(p_modifier2))->GetReferenceTargetArea();
            TS_ASSERT_DELTA(reference_target_area, 14.3, 1e-9);

            double age_to_start_growing = (static_cast<DeathClockTargetAreaModifier<2>*>(p_modifier2))->GetAgeToStartGrowing();
            TS_ASSERT_DELTA(age_to_start_growing, 0.7, 1e-9);

            double growth_rate = (static_cast<DeathClockTargetAreaModifier<2>*>(p_modifier2))->GetGrowthRate();
    ...
    void TestDeathClockTargetAreaModifierOutputParameters()
    {
        EXIT_IF_PARALLEL;
        std::string output_directory = "TestDeathClockTargetAreaModifierOutputParameters";
        OutputFileHandler output_file_handler(output_directory, false);

        MAKE_PTR(DeathClockTargetAreaModifier<2>, p_modifier);
        TS_ASSERT_EQUALS(p_modifier->GetIdentifier(), "DeathClockTargetAreaModifier-2");

        p_modifier->SetReferenceTargetArea(17.9);
        p_modifier->SetAgeToStartGrowing(5.8);
        p_modifier->SetGrowthRate(1.3);

        out_stream modifier_parameter_file = output_file_handler.OpenOutputFile("DeathClockTargetAreaModifier.parameters");
...
#endif // TESTDEATHCLOCKTARGETAREAMODIFIER_HPP_
