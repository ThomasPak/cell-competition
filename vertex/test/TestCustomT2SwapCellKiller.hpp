#ifndef TESTCUSTOMT2SWAPCELLKILLER_HPP
#define TESTCUSTOMT2SWAPCELLKILLER_HPP
 ...
 */
class TestCustomT2SwapCellKiller : public AbstractCellBasedTestSuite
{
public:
    ...
    void TestKillerForT2Swap()
    {
        ...
        std::vector<unsigned> t2_ancestors = cell_population.GetAncestorsOfT2Swaps();
        std::vector<bool> t2_is_apoptotics = cell_population.GetIsApoptoticsOfT2Swaps();
        TS_ASSERT_EQUALS(t2_ancestors.size(), 1u);
        TS_ASSERT_EQUALS(t2_is_apoptotics.size(), 1u);
        TS_ASSERT_EQUALS(t2_ancestors[0], 0);
        TS_ASSERT_EQUALS(t2_is_apoptotics[0], true);
        ...
        // Test T2 swap Ancestor clearing
        cell_population.ClearAncestorsAndIsApoptoticsOfT2Swaps();
        t2_ancestors = cell_population.GetAncestorsOfT2Swaps();
        t2_is_apoptotics = cell_population.GetIsApoptoticsOfT2Swaps();
        TS_ASSERT_EQUALS(t2_ancestors.size(), 0u);
        TS_ASSERT_EQUALS(t2_is_apoptotics.size(), 0u);
    ...
    void TestKillerForT2SwapInSimulation()
    {
        ...
        simulator.SetOutputDirectory("TestCustomT2SwapCellKillerInSimulation");
    ...
    void TestKillerForMultipleT2Swaps()
    {
        ...
        std::vector<unsigned> t2_ancestors = cell_population.GetAncestorsOfT2Swaps();
        std::vector<bool> t2_is_apoptotics = cell_population.GetIsApoptoticsOfT2Swaps();
        TS_ASSERT_EQUALS(t2_ancestors.size(), 2u);
        TS_ASSERT_EQUALS(t2_is_apoptotics.size(), 2u);
        TS_ASSERT_EQUALS(t2_ancestors[0], 4);
        TS_ASSERT_EQUALS(t2_ancestors[1], 5);
        TS_ASSERT_EQUALS(t2_is_apoptotics[0], true);
        TS_ASSERT_EQUALS(t2_is_apoptotics[1], false);
    ...
    void TestKillerForMultipleT2SwapInSimulation()
    {
        ...
        // make a simulator
        OffLatticeSimulation<2> simulator(cell_population);
        simulator.SetOutputDirectory("TestCustomT2SwapCellKillerMultipleT2SwapsInSimulation");
        simulator.SetEndTime(0.003);
    ...
    void TestCustomT2SwapCellKillerException()
    {
        // Create a cell population whose type should not be used with a CustomT2SwapCellKiller
        ...
        // Test that the correct exception is thrown if we try to use a CustomT2SwapCellKiller with the population
        TS_ASSERT_THROWS_THIS(MAKE_PTR_ARGS(CustomT2SwapCellKiller<2>, p_killer, (&population)),
            "A CustomT2SwapCellKiller should only be used together with a CustomVertexBasedCellPopulation.");
    }

    void TestArchivingOfCustomT2SwapCellKiller()
    {
...

        {
            ...
            CustomT2SwapCellKiller<2>* const p_cell_killer = &cell_killer;
...

        {
            ...
            CustomT2SwapCellKiller<2>* p_cell_killer;
...
#endif // TESTCUSTOMT2SWAPCELLKILLER_HPP
