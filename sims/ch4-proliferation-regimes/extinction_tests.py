...
#
## Helper function
#def run_extinction_uniform_simulation(Tdeath, initial_cell_count, seed=None):
#
#    # We create a random_state seeded with seed + 1 to sample the initial
#    # conditions in order to avoid correlations with the simulation.
#    if not seed is None:
...
#    thetas = np.array([ 4/7, 3/5, 2/3, 3/4, 4/5 ])
#    Tdeaths = base_rate * (tG1 - 0.5 * r + thetas * r)
...
#            zip(Tdeath_data, initial_cell_count_data, seed_data):
#
#        sim_data = run_extinction_uniform_simulation(Tdeath, initial_cell_count, seed)
#
#        status = sim_data.get_status()
#        t_events = sim_data.get_t_events()
#        cell_count = sim_data.get_cell_count()
...
#        })
#
#    df.to_csv('mc-extinction-uniform-data.csv', index_label='simulation_id')
